...
public class HComplainListAdapter extends BaseAdapter{
	...
	public HComplainListAdapter(Context context,List<Complain> newsList) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			convertView = layoutInflater.inflate(R.layout.complain_list_item, null);
		...
		if(0==complain.getState()){//未处理
			...
			holder.state.setText("ٴبىرجاقتىلى ەتىلمەگەن");
		}else if(1==complain.getState()){//处理中
			holder.state.setTextColor(Color.GREEN);
			holder.state.setText("ٴبىر جاقتىلى ەتۋ بارىسىندا");
		}else{//已处理
			holder.state.setTextColor(Color.GRAY);
			holder.state.setText("ٴبىر جايلى ەتىلدى");
		}
	...
	private static class ViewHolder {
		...
		private TextView title,time,state;
