...
public class MChangePhoneActivity extends Activity {
	...
	Timer timer;
	int time = 60;//用于计时	
	MongolTextView head_change_phone;
	
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_change_phone_m);
		...
		handler = new Handler() {
			public void handleMessage(Message msg) {
				...
				case 4:
					...
					Toast.makeText(MChangePhoneActivity.this, result, Toast.LENGTH_SHORT)
							.show();
					break;
				default:
					break;
	...
	private void initWight() {
		...
		head_change_phone = (MongolTextView)findViewById(R.id.head_change_phone);
	}
	
	
	private void initText() {
		// TODO Auto-generated method stub
		head_change_phone.setText("ᠤᠲᠠᠰᠤᠨ ᠳ᠋ᠤᠭᠠᠷ ᠶ᠋ᠢᠡᠨ ᠰᠣᠯᠢᠬᠤ");//换绑手机
		submitBtn.setText("ᠪᠠᠲᠤᠯᠠ");//确认
			...
			if (v == backBtn)
				...
				if(isMobileNO(newPhone.getText().toString())){
					...
					dialog = MyDialog.createLoadingDialog(
							MChangePhoneActivity.this, "数据提交中...");
					dialog.show();
				}
			}else if(v==get_code){
				...
				if(isMobileNO(phone)){
					...
					MyToast.showToast(MChangePhoneActivity.this, "يانفون نومۇرىنى كىرگۈزۈڭ", 1);
			}
		}
	}
    ...
    class UpdateThread extends Thread {
		...
		public void run() {
			...
			if (user == null) {
				...
				Tools.saveUserInfo(user, MChangePhoneActivity.this);
				data.putInt("ret", 1);
			}
