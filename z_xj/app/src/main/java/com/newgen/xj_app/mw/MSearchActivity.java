...
import com.newgen.adapter.MSearchListAdapter;
...
public class MSearchActivity extends Activity implements OnClickListener{
	...
	private MongolTextView search,search_record;
	private ImageView del_record;
	private HorizontalListView record_list;
	
	private List<String> recordsList = new ArrayList<String>();
	private MSearchListAdapter adapter;
	...
	private void initWight() {
		...
		adapter = new MSearchListAdapter(MSearchActivity.this, recordsList,isNight);
		backButton = (ImageView) findViewById(R.id.back);
		search_key = (EditText) findViewById(R.id.search_key);
		search = (MongolTextView) findViewById(R.id.search);
		search_record = (MongolTextView)findViewById(R.id.search_record);
		search.setText("ᠨᠡᠩᠵᠢᠯ");
		search_record.setText("ᠬᠠᠢᠢᠯᠳᠠ ᠲᠡᠮᠳᠡᠭᠯᠡᠯ");
		del_record = (ImageView) findViewById(R.id.del_record);
		record_list = (HorizontalListView) findViewById(R.id.record_list);
	...
	public void onClick(View v) {
		...
		if(v ==backButton)
			...
			if(isLaw(search_text)){
				...
				Intent intent = new Intent(MSearchActivity.this,MSearchResultActivity.class);
				intent.putExtra("key", search_text);
				startActivity(intent);
			}else{
				Toast.makeText(MSearchActivity.this, "请输入关键字，空格不算哦！", 5).show();
			}
	...
	private void getSearchRecord() {
		// TODO Auto-generated method stub
		String jStr = SharedPreferencesTools.getValue(MSearchActivity.this,
	...
	private void saveSearchRecord(String key) {
		...
		SharedPreferencesTools.setValue(MSearchActivity.this, "search_record", 
