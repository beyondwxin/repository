...
import com.newgen.adapter.MImgNewsDetailAdapter;
...
public class MImgNewsDetailActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_img_news_detail_m);
	...
	class OnClick implements View.OnClickListener {
		...
		public void onClick(View v) {
			if (v == backButton) {
				...
				share.showShare(false, null, MImgNewsDetailActivity.this, news);
			}else if(v==commentButton){
				Intent intent = new Intent(MImgNewsDetailActivity.this,
				...
				if(isCollected){
					int result = hleper.deleteCollectNews(news.getId(),isEpaper,MImgNewsDetailActivity.this);
					if(result>0){
						isCollected = false;
					...
					hleper.addCollect(newsMix, MImgNewsDetailActivity.this);
				...
				SqlHleper hleper = new SqlHleper();
				if(isSupported){
					int result = hleper.deleteSupportNews(news.getId(), MImgNewsDetailActivity.this);
					if(result>0){
						isSupported = false;
						thumb_up.setBackgroundResource(R.drawable.thumb_up);
						Toast.makeText(getApplicationContext(), "取消点赞",
								Toast.LENGTH_SHORT).show();
						Log.e("1111", hleper.getSupportNewsList(MImgNewsDetailActivity.this, 1, 10)+"");
					}else
						Toast.makeText(getApplicationContext(), "取消失败",
								Toast.LENGTH_SHORT).show();
				}else{
					...
					hleper.addSupport(news, MImgNewsDetailActivity.this);
	...
	private class GetData extends AsyncTask<Object, Object, Integer> {
		...
		protected Integer doInBackground(Object... arg0) {
			...
			SqlHleper hleper = new SqlHleper();
			isSupported = hleper.findSupportByNewsId(newsID, MImgNewsDetailActivity.this);
			isCollected = hleper.findCollectByNewsId(newsID, isEpaper, MImgNewsDetailActivity.this);
		...
		protected void onPostExecute(Integer result) {
			...
			case 1:
				...
				if (news.getListpic() != null && news.getListpic().size() > 0) {
					...
					MImgNewsDetailAdapter adapter = new MImgNewsDetailAdapter(
							imgList, news.getTitle(),
							MImgNewsDetailActivity.this);
			...
			public void onClick(View arg0) {
				...
				if (PublicValue.USER != null) {
					PostCommentThread postCommentThread = new PostCommentThread(
							MImgNewsDetailActivity.this.newsID, clickedReplyId,
	...
	private static class MHandler extends Handler {
		...
		WeakReference<MImgNewsDetailActivity> mActivity;
		
		MHandler(MImgNewsDetailActivity activity) {
			mActivity = new WeakReference<MImgNewsDetailActivity>(activity);
		}
